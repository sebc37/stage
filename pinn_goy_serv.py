...
class Train_PINN():
    ...
    def train(self):
        ...
        for iteration in tqdm.tqdm(range(self.nbr_iteration)):
            ...
            u_pd = torch.cat(tuple_u_pd,1)            
        
            GOY_physics = torch.zeros(Npts,(k_max-2 - (k_min+2)))
            #print(GOY_physics.shape)
            for i in range (k_min+2,k_max-2):
                GOY_physics[:,i-k_min-2] = u_t[:,i] -K[i]*u_pd[:,i+1]*u_pd[:,i+2] +K[i]*eps/lmb*u_pd[:,i-1]*u_pd[:,i+1] - K[i]*((eps-1)/lmb**2)*u_pd[:,i-2]*u_pd[:,i-1] + nu*K[i]*K[i]*u_pd[:,i] # à confirmer
...
def test_loss(Data_train,grille_datatset):
    ...
    grid_train_data = grille_datatset.grille
    grid_train_data.requires_grad = True
    b = Data_train[:,k_min:k_max]
    tensor_data = torch.tensor(Data_train[:,k_min:k_max],requires_grad=True).T#view(Npts,k_max-k_min)
    tensor_split = torch.split(tensor_data,k_max-k_min,1)
    u = torch.cat(tuple(k for k in tensor_split),0)
    
    GOY_physics = torch.zeros(Npts,(k_max-2 - (k_min+2)))
    torch.enable_grad()
    u_t = torch.autograd.grad(u, grid_train_data, torch.ones_like(u), create_graph=True,allow_unused=True)[0]#[:,1:2]
            #print(GOY_physics.shape)

    u_split = torch.split(u_t,Npts)
    u_t = torch.cat(tuple(k for k in u_split),1)

    for i in range (k_min+2,k_max-2):
        GOY_physics[:,i-k_min-2] = u_t[:,i] -K[i]*u[:,i+1]*u[:,i+2] +K[i]*eps/lmb*u[:,i-1]*u[:,i+1] - K[i]*((eps-1)/lmb**2)*u[:,i-2]*u[:,i-1] + nu*K[i]*K[i]*u[:,i] # à confirmer
...
U = model(grid_dataset.grille)
U_split = torch.split(U,Npts)
U = torch.cat(tuple(k for k in U_split),1)
U = U.detach().numpy()#.detach().numpy().reshape(t_max-t_min,k_max-k_min)
...
for i in range(U.shape[1]):
    ...
    plt.ylabel('Velocity')
    plt.legend()
    plt.savefig(f"prediction_u{i}.png")
