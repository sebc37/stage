...
Data_filtered ,Var_mode,Std_mode,perc,Data_train,mean = filter_mode(Data_shell,5,10,int(0.1*100100),0.001,123456)
...
class Train_PINN():
    ...
    def train(self):
        ...
        for iteration in tqdm.tqdm(range(self.nbr_iteration)):
            ...
            GOY_physics = torch.zeros(Npts,(k_max-2 - (k_min+2))).to(device)
...
def test_loss(Data_train,grille_datatset):
    ...
    data = Data_train[:,k_min:k_max]
    tensor_u = torch.ones((k_max-k_min)*Npts)
    ...
    for t in range(1,Npts):
        u_t[t,:] = (u_t[t,:]-u_t[t-1,:])/dt

    # for k in range((k_max-k_min)*Npts):
    #         i = k%(Npts)
    #         j = k//(Npts)
    #         tensor_u[k] = data[i,j]
    # u = tensor_u
    # u.requires_grad = True#torch.cat(tuple(k for k in tensor_split),0)
    ...
    for i in range (k_min+2,k_max-2):
            GOY_physics[:,i-k_min-2] = u_t[:,i] -K[i]*u[:,i+1]*u[:,i+2] +K[i]*eps/lmb*u[:,i-1]*u[:,i+1] - K[i]*((eps-1)/lmb**2)*u[:,i-2]*u[:,i-1] + nu*K[i]*K[i]*u[:,i] #deltat =1
...
learning_rate,nbr_iteration,w_1,w_2,w3,w_4 = 0.001,300,1,1,1,1
